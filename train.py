...
parser.add_argument('--beta','-b', default=2, type=float)
...
parser.add_argument('--soft', '-S', type=float, default=2.0)
...
logdir = os.path.join(args.saveroot, args.dataset+'_'+args.model, 'temp_'+ str(args.temp)+'soft_'+str(args.soft)+'_lamda_'+str(args.lamda)+'_alpha_'+str(args.alpha)+'_beta_'+str(args.beta))
...
s_w = [f.shape[1]*f.shape[2]*f.shape[3] for f in feat[:-1]]
t_n = feat[-1].shape[1]
#t_w = feat[-1].shape[1]*feat[-1].shape[2]*feat[-1].shape[3]
criterion_kd = SemCKDLoss(args)
self_attention = SelfAttention(args.feat_dim, s_n, s_w, t_n, num_class, args.soft)    
...
def train(epoch, module_list):
    ...
    for batch_idx, (inputs, labels) in enumerate(trainloader):
        ...
        s_value, f_target, s_pred, weight = module_list[1](feat[0:-1], teacher_feature)
        loss_kd = criterion_kd(s_value, f_target, s_pred, labels, weight)
